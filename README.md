...
项目面向个人创作者、课程剪辑、访谈整理和跨语言内容制作场景，将视频转写、字幕翻译、模型准备、依赖检查和任务输出集中在同一个本地工作台中完成。
...
- API Key 保存在应用用户数据目录的本地 SQLite 数据库中，不应写入代码、日志或 `.env` 文件。
...
- macOS Apple Silicon：优先使用本机已安装或随应用内置的 arm64 `ffmpeg` 与 `whisper-cli`；缺失时可从官方源码自动编译 FFmpeg 与 Metal 版 whisper.cpp，要求 macOS 11.0+。
